...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("ServiceName", "AlertService")
    .WriteTo.Console()
    .CreateLogger();
...
app.MapGet("/", () => "Alert Service - gRPC endpoint available");
...
try
{
    Log.Information("Starting Alert Service");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Alert Service terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
