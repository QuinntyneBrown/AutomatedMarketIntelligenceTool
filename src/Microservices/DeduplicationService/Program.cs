...
using DeduplicationService.Services;
...
Log.Logger = new LoggerConfiguration()
    ...
    .Enrich.WithProperty("ServiceName", "DeduplicationService")
    .WriteTo.Console()
    .CreateLogger();
...
var serviceName = "DeduplicationService";
...
var connectionString = builder.Configuration.GetConnectionString("DeduplicationDb")
    ?? "Data Source=deduplication-service.db";
...
// Map gRPC services
app.MapGrpcService<DeduplicationGrpcService>();
...
app.MapGet("/", () => "Deduplication Service - gRPC endpoint available");
...
try
{
    Log.Information("Starting Deduplication Service");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Deduplication Service terminated unexpectedly");
}
finally
{
    Log.CloseAndFlush();
