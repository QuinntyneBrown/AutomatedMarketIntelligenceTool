...
public class TrueCarScraper : BaseScraper
{
    public override string SiteName => "TrueCar.com";

    public TrueCarScraper(ILogger<TrueCarScraper> logger)
        : base(logger)
    {
    }
    ...
    {
        var baseUrl = new StringBuilder("https://www.truecar.com/used-cars-for-sale/listings");
        ...
        if (parameters.RadiusKilometers.HasValue)
        {
            ...
            queryParams.Append($"&searchRadius={miles}");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await page.WaitForSelectorAsync("[data-test='cardContent'], .card-content, .listing-card", new PageWaitForSelectorOptions
            {
                Timeout = 10000
            });

            var listingElements = await page.QuerySelectorAllAsync("[data-test='cardContent'], .card-content, .listing-card");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var titleElement = await element.QuerySelectorAsync("[data-test='vehicleCardTitle'], .vehicle-card-title, h3");
            var title = titleElement != null ? await titleElement.InnerTextAsync() : string.Empty;

            var priceElement = await element.QuerySelectorAsync("[data-test='vehicleCardPrimaryPrice'], .vehicle-card-price");
            var priceText = priceElement != null ? await priceElement.InnerTextAsync() : "0";
            var price = ParsePrice(priceText);

            var linkElement = await element.QuerySelectorAsync("a[href*='/used/'], a[href*='/listing/']");
            ...
            var listingUrl = href.StartsWith("http", StringComparison.OrdinalIgnoreCase)
                ? href
                : $"https://www.truecar.com{href}";

            var externalId = ExtractExternalId(listingUrl);

            var mileageElement = await element.QuerySelectorAsync("[data-test='vehicleCardMileage'], .mileage");
            var mileageText = mileageElement != null ? await mileageElement.InnerTextAsync() : string.Empty;
            var mileage = ParseMileage(mileageText);

            var locationElement = await element.QuerySelectorAsync("[data-test='vehicleCardLocation'], .location");
    ...
    protected override async Task<bool> HasNextPageAsync(IPage page, CancellationToken cancellationToken)
    {
        try
        {
            var nextButton = await page.QuerySelectorAsync("[data-test='nextPage']:not([disabled]), .pagination .next:not(.disabled)");
    ...
    protected override string ExtractExternalId(string url)
    {
        ...
        try
        {
            var match = Regex.Match(url, @"/(?:used|listing)/[^/]+/([^/\?]+)");
            if (match.Success)
            {
