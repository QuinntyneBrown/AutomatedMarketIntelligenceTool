...
public class CarvanaScraper : BaseScraper
{
    public override string SiteName => "Carvana.com";

    public CarvanaScraper(ILogger<CarvanaScraper> logger)
        : base(logger)
    {
    }
    ...
    {
        var baseUrl = new StringBuilder("https://www.carvana.com/cars");
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await page.WaitForSelectorAsync("[data-qa='base-inventory-card'], .inventory-card, .vehicle-card", new PageWaitForSelectorOptions
            {
                Timeout = 10000
            });

            var listingElements = await page.QuerySelectorAllAsync("[data-qa='base-inventory-card'], .inventory-card, .vehicle-card");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var titleElement = await element.QuerySelectorAsync("[data-qa='base-inventory-vehicle-title'], .vehicle-title, h3");
            var title = titleElement != null ? await titleElement.InnerTextAsync() : string.Empty;

            var priceElement = await element.QuerySelectorAsync("[data-qa='base-inventory-price'], .price");
            var priceText = priceElement != null ? await priceElement.InnerTextAsync() : "0";
            var price = ParsePrice(priceText);

            var linkElement = await element.QuerySelectorAsync("a[href*='/vehicle/']");
            ...
            var listingUrl = href.StartsWith("http", StringComparison.OrdinalIgnoreCase)
                ? href
                : $"https://www.carvana.com{href}";

            var externalId = ExtractExternalId(listingUrl);

            var mileageElement = await element.QuerySelectorAsync("[data-qa='base-inventory-mileage'], .mileage");
    ...
    protected override async Task<bool> HasNextPageAsync(IPage page, CancellationToken cancellationToken)
    {
        try
        {
            var nextButton = await page.QuerySelectorAsync("[data-qa='next-page']:not([disabled]), .pagination .next:not(.disabled)");
    ...
    private static string ExtractExternalId(string url)
    {
        ...
        try
        {
            var match = Regex.Match(url, @"/vehicle/(\d+)");
            if (match.Success)
            {
