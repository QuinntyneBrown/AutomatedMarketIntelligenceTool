...
public class CarFaxScraper : BaseScraper
{
    public override string SiteName => "CarFax.ca";

    public CarFaxScraper(ILogger<CarFaxScraper> logger)
        : base(logger)
    {
    }
    ...
    {
        var baseUrl = new StringBuilder("https://www.carfax.ca/en/used-cars-for-sale");
        ...
        {
            queryParams.Append($"&postalCode={HttpUtility.UrlEncode(parameters.PostalCode)}");
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        try
        {
            await page.WaitForSelectorAsync(".listing-item, .vehicle-card, [data-test='listing']", new PageWaitForSelectorOptions
            {
                Timeout = 10000
            });

            var listingElements = await page.QuerySelectorAllAsync(".listing-item, .vehicle-card, [data-test='listing']");
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            var titleElement = await element.QuerySelectorAsync(".listing-title, .vehicle-name, h3");
            ...
            var vinElement = await element.QuerySelectorAsync(".vin, [data-vin]");
            var vin = vinElement != null ? await vinElement.InnerTextAsync() : null;
            if (string.IsNullOrWhiteSpace(vin))
            {
                vin = vinElement != null ? await vinElement.GetAttributeAsync("data-vin") : null;
            }
